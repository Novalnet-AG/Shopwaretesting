...
class NovalnetHelper
{
    ...
    public function getVersionInfo(Context $context) : string
    {
        ...
        return $this->shopVersion . '-NN' . '12.1.3';
    }
    ...
    public function formBasicComments(array $input, bool $isError = false) : string
    {
        ...
        if (! empty($input ['transaction']['tid'])) {
            ...
            if (! empty($input ['transaction'] ['test_mode'])) {
                $comments .= $this->newLine . $this->translator->trans('NovalnetPayment.text.testOrder');
            }
    ...
    public function formBankDetails(array $input) : string
    {
        ...
        if ($input['transaction']['status'] === 'PENDING' && in_array($input['transaction']['payment_type'],['GUARANTEED_INVOICE', 'INSTALMENT_INVOICE'])) {
            ...
            if ($input['transaction']['payment_type'] !== 'INSTALMENT_INVOICE') {
                ...
                $comments .=  $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.paymentReference'), '1', $input ['transaction']['invoice_ref']) . $this->newLine;
                
                if(! empty($input ['transaction']['invoice_ref']))
                {
                    /* translators: %s:  TID */
                    $comments .=  sprintf($this->translator->trans('NovalnetPayment.text.paymentReference'), '2', 'TID '. $input ['transaction']['tid']) . $this->newLine;
                }
            } else {
                /* translators: %s:  TID */
                $comments .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.instalmentPaymentReference'), 'TID '. $input ['transaction']['tid']) . $this->newLine;
            }
    ...
    public function formMultibancoDetails(array $input) : string
    {
        ...
        $amountInBiggerCurrencyUnit = $this->amountInBiggerCurrencyUnit( $input ['transaction']['amount'], $input ['transaction']['currency'] );
            $comments .= $this->newLine . $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.multibancoReference'), $amountInBiggerCurrencyUnit). $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.instalmentPaymentReference'), $input['transaction']['partner_payment_reference']);
        }
